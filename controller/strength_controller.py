...
def create(data: StrengthCreateDTO, db: Session = Depends(get_db)):
    """יצירת רשומת חוזק חדשה"""
    return StrengthRepository(db).create(data)


@router.put("/{strength_id}", response_model=StrengthResponseDTO)
def update(strength_id: int, data: StrengthCreateDTO, db: Session = Depends(get_db)):
    """עדכון רשומת חוזק"""
...
def delete(strength_id: int, db: Session = Depends(get_db)):
    """מחיקת רשומת חוזק"""
    if not StrengthRepository(db).delete(strength_id):
        raise HTTPException(status_code=404, detail=f"חוזק {strength_id} לא נמצא")
    return None
