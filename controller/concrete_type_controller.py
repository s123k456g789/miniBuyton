...
def create(data: ConcreteTypeCreateDTO, db: Session = Depends(get_db)):
    """יצירת סוג בטון חדש"""
    return ConcreteTypeRepository(db).create(data)


@router.put("/{type_id}", response_model=ConcreteTypeResponseDTO)
def update(type_id: int, data: ConcreteTypeCreateDTO, db: Session = Depends(get_db)):
    """עדכון סוג בטון"""
...
def delete(type_id: int, db: Session = Depends(get_db)):
    """מחיקת סוג בטון"""
    if not ConcreteTypeRepository(db).delete(type_id):
        raise HTTPException(status_code=404, detail=f"סוג בטון {type_id} לא נמצא")
    return None
