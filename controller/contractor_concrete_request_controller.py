...
#from service.contractor_concrete_request_service import ContractorConcreteRequestService
from service.contractor_concrete_service import ContractorConcreteService
...
@router.post("/", response_model=ContractorConcreteRequestResponseDTO, status_code=201)
def create_offer(data: ContractorConcreteRequestCreateDTO, db: Session = Depends(get_db)):
    service = ContractorConcreteService(db)
    return service.create_offer(data)
# @router.post("/send", response_model=ContractorConcreteRequestResponseDTO, status_code=201)
# def create_offer(data: ContractorConcreteRequestCreateDTO, db: Session = Depends(get_db)):
#     service = ContractorConcreteService(db)
#     return service.create_offer(data)

@router.put("/{request_id}", response_model=ContractorConcreteRequestResponseDTO)
def update_offer(request_id: int, data: ContractorConcreteRequestCreateDTO, db: Session = Depends(get_db)):
    service = ContractorConcreteService(db)
    updated = service.update_offer(request_id, data)
    if not updated:
        raise HTTPException(status_code=404, detail=f"הצעה {request_id} לא נמצאה")
    return updated
